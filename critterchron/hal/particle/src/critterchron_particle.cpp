...
// Two segment shapes accepted:
//   `HH:MM-HH:MM:max`         → override max only, min_brightness unchanged
//   `HH:MM-HH:MM:min-max`     → override BOTH ends (use to hard-pin segment)
// The two-value form is parsed first; on no-match, falls back to single-value.
static bool parse_brightness_schedule(const char* s) {
    ...
    while (*p && seg_idx < SCHEDULE_MAX_SEGS) {
        ...
        // Try the two-value `HH:MM-HH:MM:min-max` form first.
        if (sscanf(p, "%d:%d-%d:%d:%d-%d%n",
                   &sh, &sm, &eh, &em, &low, &high, &n) == 6) {
            two_value = true;
        } else {
            // Fall back to single-value `HH:MM-HH:MM:max`.
            n = 0;
        ...
        s_sched_segs[seg_idx].min_bri   = (uint8_t)low;  // 0 if single-value
        ++seg_idx;
        p += n;
    }
...
// `out_min_bri` (optional) receives the segment's min_bri override: 0 if
// the segment used the single-value form (no min override), 1..255 if the
// two-value form was used. Caller is responsible for ensuring
// s_sched_parse_ok is true and Time is valid before consulting.
...
void loop() {
    ...
    if (now - last_light_ms >= 200) {
        ...
        if (s_sched_parse_ok && s_sched_count > 0 && Time.isValid()) {
            ...
            if (sched_max >= 0) {
                max_b = sched_max;
                // Two-value form (`HH:MM-HH:MM:min-max`) also pins min.
                // sched_min == 0 means single-value form — keep the
                // device's `min_brightness` as already pulled above.
