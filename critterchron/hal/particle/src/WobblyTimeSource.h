...
public:
    ...
    time_t wall_now() const override {
        ...
        // If the real clock jumped (cloud resync after a long offline spell),
        // chasing the drift would take minutes. Snap to a fresh target.
        //
        // "Way outside the configured range" = twice the larger-magnitude
        // bound. std::abs on both bounds keeps the gate symmetric across
        // positive-only ("fast clock"), negative-only ("slow clock"), and
        // mixed ("drift around real") configurations. The previous form
        // `max_s * 2.0` assumed max_s was the largest-magnitude bound —
        // broke for negative bounds (e.g. max_s=-5 gave threshold -10,
        // which |offset| trivially exceeds → reseed every call → wobble
        // can never converge).
        ...
        // large jumps caught by the jump_threshold reseed gate above.
