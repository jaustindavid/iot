...
// ESP32-C6's RMT peripheral has no DMA. FastLED's per-chip default
// for `FASTLED_RMT_WITH_DMA` (in led_strip/defs.h) lists ESP32/S2/
// ESP8266/C3 as no-DMA but OMITS C6, so C6 falls through to the
// "DMA assumed" branch — `led_strip_new_rmt_device_with_buffer` calls
// `rmt_new_tx_channel` with `with_dma=true` → ESP_ERROR_CHECK abort
// → crashloop in FastLEDSink::begin(). The fix is forcing
// FASTLED_RMT_WITH_DMA=0 at compiler level (in hal/esp32c6/Makefile)
// so every FastLED .cpp sees it; an in-header define here only reaches
// the .ino's translation unit and FastLED's library files miss it.
