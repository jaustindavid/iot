...
#define LISTENNEXT_ADDY 7
#define CHECKBIT 0xad
...
uint8_t listenNext = CHECKBIT;

void setListenNext(const bool next) {
    listenNext = CHECKBIT;
    Serial.print("listenNext: ");
    Serial.print(listenNext == CHECKBIT ? "true" : "false");
    Serial.println();
    EEPROM.put(LISTENNEXT_ADDY, next ? CHECKBIT : 0);
} // setListenNext(bool)


int listenCallback(String data) {
    setListenNext(true);
    return 1;
} // listenCallback(data)


// force the SoftAP to fire up
void runSoftAP() {
    Serial.print("Running the soft AP...");
...
// call setupSoftAP() in setup() to enable this to magically restart
// on the next reboot
// use the function "enableAP" via Particle console
void setupSoftAP() {
    Particle.variable("listenNext", listenNext);
    Particle.function("enableAP", listenCallback);
    EEPROM.get(LISTENNEXT_ADDY, listenNext);

    Serial.print("listenNext: ");
    Serial.print(listenNext == CHECKBIT ? "true" : "false");
    Serial.println();
    
    if (listenNext == CHECKBIT) {
        setListenNext(false);
        runSoftAP();
    } 
} // setupSoftAP()
